...
**Trade-off:** that unpacks more than it needs to, but narrowing it saves no space. Of 146MB unpacked, 137MB is `Frameworks/` and 9.3MB the SDK binary, both of which must stay outside to be executable, leaving ~92KB to move. The gain would be asar integrity coverage for `index.js`, which `EnableEmbeddedAsarIntegrityValidation` and `OnlyLoadAppFromAsar` currently cannot reach.
